...
| 6 | **Bookings & seat inventory** — backend: request/accept/decline, concurrency-safe reservation, cancellation. Mobile: reserve-seat flow, driver's incoming-requests screen. | 🔶 Backend done, mobile pending |
...
**Phase 6 mobile**: a reserve-seat flow from the trip-detail screen (pick boarding/alighting stops + seat count, confirm), a Bookings tab showing the passenger's own bookings, and the driver's incoming-requests screen (accept/decline) on a trip. Then **Phase 7 — Payments abstraction**.
