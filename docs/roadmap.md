...
| 7 | **Payments abstraction** — backend: `IPaymentGateway`, mock provider, fare/fee/payout split, refunds. Mobile: payment status/receipt UI. | 🔶 Backend done, mobile pending |
...
**Phase 7 mobile**: payment status/receipt UI on a confirmed booking's detail screen (fare, platform fee, driver payout, provider reference), and a refund action on the driver's side. Then **Phase 8 — Ratings & reviews**.
